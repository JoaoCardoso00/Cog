...
impl Environment {
    ...
    pub fn declare_variable(&mut self, variable_name: String, value: RuntimeValue) -> RuntimeValue {
    ...
    pub fn peek_variable(&mut self, variable_name: String) -> RuntimeValue {
        let environment = self.resolve(variable_name.clone());
        environment.variables.get(&variable_name).unwrap().clone()
    }

    pub fn resolve(&mut self, variable_name: String) -> Environment {
        if self.variables.contains_key(&variable_name) {
            return self.clone();
        }

        match self.parent {
            Some(ref mut parent) => parent.resolve(variable_name),
